    ...
    >
      <body className="min-h-full">
        ...
        {/* iPhone status-bar / Dynamic Island cover. Body-level, very high
            ...
            Height = clamp(8px, env(safe-area-inset-top), 12px) — a hairline strip,
            per request. env(safe-area-inset-top) resolves to 0 in some iOS Safari
            states even on a Dynamic Island phone, so the 8px floor keeps a solid
            band at the very top; the 12px cap keeps it minimal (and guards against
            a bogus-large env). The navbar's own top padding still clears the iOS
        ...
        <div
          ...
          style={{ height: "clamp(8px, env(safe-area-inset-top), 12px)" }}
        />
      </body>
    </html>
  );
